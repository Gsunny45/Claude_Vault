#!/usr/bin/env bash
# ============================================================
# go-vision.sh — Hardened launcher for Gemma 4 E4B (multimodal / vision)
# Run from INSIDE proot Ubuntu:
#   proot-distro login ubuntu
#   bash /sdcard/go-vision.sh
# ============================================================
set -u
...
CTX=2048
NPRED=512
MIN_FREE_RAM_MB=6144   # 6 GB threshold for multimodal (model + mmproj)
LOG_FILE="/tmp/go-gemma-vision.log"
...
# 1. Existence checks
# -----------------------------------------------------------
log "=== Starting go-vision.sh (Gemma 4 E4B multimodal) ==="
...
if [ ! -f "$MMPROJ" ]; then
    log "ERROR: mmproj (multimodal projector) file not found: $MMPROJ"
    log "       Run setup-models.sh to download it, or use go.sh for text-only mode."
    exit 1
fi

log "Binary found:  $SERVER"
log "Model found:   $MODEL"
log "mmproj found:  $MMPROJ"
...
if [ "$FREE_RAM_MB" -gt 0 ] && [ "$FREE_RAM_MB" -lt "$MIN_FREE_RAM_MB" ]; then
    ...
    log "         for the multimodal Gemma 4 E4B + mmproj setup (~+1 GB over text-only)."
    log "         The server may be slow, swap heavily, or be killed by the OOM killer."
    echo ""
    echo "Tip: 'go.sh' (text-only) needs less RAM if vision isn't required."
...
cleanup() {
    ...
    log "=== go-vision.sh shutdown complete ==="
    exit 0
}
...
# 5. Launch
# -----------------------------------------------------------
log "Starting Gemma 4 E4B (multimodal) on ${HOST}:${PORT} ..."
log "Command: $SERVER -m $MODEL --mmproj $MMPROJ --host $HOST --port $PORT -c $CTX -n $NPRED --threads $THREADS"

"$SERVER" -m "$MODEL" --mmproj "$MMPROJ" --host "$HOST" --port "$PORT" -c "$CTX" -n "$NPRED" --threads "$THREADS" &
SERVER_PID=$!
